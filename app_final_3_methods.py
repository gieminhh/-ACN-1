...
CHECKPOINT_CANDIDATES = [
    # Đây là các file model đã train sẵn. App chỉ load lại để suy luận,
    # nên chạy app là có kết quả ngay, không cần train từ đầu.
...
def load_env_and_model():
    """Tạo môi trường CVRP và nạp mô hình PPO + Attention đã train.

    Hàm này là bước đầu tiên của app. Nếu tìm thấy checkpoint, app dùng
    "bộ não" đã học sẵn; nếu không có thì tạo model mới chưa train để app
    vẫn chạy được nhưng kết quả chỉ mang tính demo.
    """
...
def flatten_best_actions(out):
    ...
    """
    # Model trả reward, trong bài toán route thì reward = -cost.
    # Reward càng lớn nghĩa là cost càng nhỏ.
...
def decode_solution(model, td, decode_type, coords, beam_width, sampling_samples, temperature, seed):
    """
    Sinh route bằng một kiểu decoder: greedy, sampling hoặc beam.

    Greedy: chọn node xác suất cao nhất.
    Sampling: bốc mẫu theo phân phối xác suất.
    Beam: giữ nhiều phương án ứng viên rồi chọn đường tốt.

    Sau khi sinh route, cost luôn được tính lại bằng solution_cost(routes, coords)
    để cả 3 phương pháp so sánh trên cùng một thước đo.
    """
    torch.manual_seed(seed)
...
def repair_candidate(candidate, coords, demands, capacity):
    ...
    """
    # Đây là bước "kiểm tra lại đường đi": thử đổi vị trí khách hàng.
    # Nếu đổi làm tổng quãng đường ngắn hơn và không quá tải thì nhận.
...
def solve_map(env, model, map_id, beam_width, sampling_samples, temperature):
    """Giải một MAP bằng 3 phương pháp và tạo kết quả để hiển thị.

    Đây là hàm trung tâm của thuật toán đề xuất trong app:
    1. Sinh nghiệm bằng Greedy và Sampling để làm baseline.
    2. Dùng Beam + Attention để lấy thứ tự khách hàng tốt hơn.
    3. Dùng Best Insertion dựng route từ thứ tự đó.
    4. Repair các nghiệm ứng viên rồi chọn nghiệm có cost nhỏ nhất.
    """
    seed = 42 + int(map_id)
    ...
    # Nói dễ hiểu: tạo vài lời giải ứng viên, sửa từng cái, rồi lấy cái rẻ nhất.
